...
def check_basicinfo(userinfo):
    infolist = ['postcode', 'mentor', 'address', 'ethnic']
    for i in infolist:
...
@app.route('/scholarshipapply/<username>', methods=['GET','POST'])
def scholarshipapply(username):
    print request.form
    if not canapply:
        return '申请时间已过'
...
@app.route('/getscholarshiplist/<condition>', methods=['GET','POST'])
def getscholarshiplist(condition):
    md = {u'普博': u'博士', u'直博': u'博士', u'硕博连读': u'博士', u'硕士': u'硕士'}
    try:
        mas_doc, suo, whichscore = condition.split('_')
        print mas_doc, suo, whichscore
    except:
        print 'condition error', condition
        mas_doc = 'doc'
        suo = ''
        whichscore = 'academic' # ''
    try:
        ...
        for tmpuser in a:
            #mas_doc
            if tmpuser.has_key('mas_doc'):
                if md.has_key(tmpuser['mas_doc']):
                    if md[tmpuser['mas_doc']] != mas_doc:
                        continue
                else:
                    print tmpuser['mas_doc']
                    continue
            else:
                continue
            #suo
            if suo != '' and tmpuser.has_key('suo'):
                if tmpuser['suo'] != suo:
                    continue
...
def getnotify():
    try:
        ...
        for tmpn in a:
            print tmpn
            tmpresult = {}
            tmpresult['description'] = tmpn['description']
            tmpresult['time'] = str(tmpn['time'])[:16]
            tmpresult['filename'] =tmpn['filename']
            result.append(tmpresult)
        return json.dumps(result)
    except:
        traceback.print_exc()

@app.route('/getgongshi', methods=['GET','POST'])
def getgongshi():
    try:
        tmp = gongshidb.find()
        a = [entry for entry in tmp]
...
def downloadnotify(filename):
    try:
        ...
        return send_from_directory(directory='notify',filename=filename.encode('utf-8'), as_attachment=True)
    except:
        traceback.print_exc()
        return u'后台错误'
...
def downloadgongshi(filename):
    try:
        print repr(filename.encode('utf-8'))
        return send_from_directory(directory='gongshi',filename=filename.encode('utf-8'), as_attachment=True)
    except:
        traceback.print_exc()
        return u'后台错误'